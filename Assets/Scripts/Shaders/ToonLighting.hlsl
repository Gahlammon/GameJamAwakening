...
half Toon(half input)
{
    ...
    return output;   //----TOON---- Function to toon the half value
}

half3 Toon3(half3 input)
{
    half3 output = half3(1, 1, 1);
    if ((input.r + input.g + input.b) / 3 < 0.02)
    {
        output = output * 0;
    }
    return output;   //----TOON---- Function to toon the half3 value
}

half4 Toon4(half4 input)
{
    half4 output = half4(1, 1, 1, 1);
    if ((input.r + input.g + input.b + input.a) / 4 < 0.02)
    {
        output = output * 0;
    }
    return output;   //----TOON---- Function to toon the half4 value
}

half3 LightingLambert(half3 lightColor, half3 lightDir, half3 normal)
{
    half NdotL = Toon(saturate(dot(normal, lightDir)));   //----TOON---- Is that even necesary? Not sure
...
half3 LightingSpecular(half3 lightColor, half3 lightDir, half3 normal, half3 viewDir, half4 specular, half smoothness)
{
    ...
    half3 specularReflection = Toon3(specular.rgb * modifier);   //----TOON---- No idea if it's even used but...
...
{
    half NdotL = saturate(dot(normalWS, lightDirectionWS));   //----TOON---- Toon the light position based lighting 
    half3 radiance = lightColor * Toon3(lightAttenuation * NdotL);   //----TOON---- Toon the outline light (at least part of it)

    half3 brdf = brdfData.diffuse;
#ifndef _SPECULARHIGHLIGHTS_OFF
    [branch] if (!specularHighlightsOff)
    {
        brdf += Toon3(brdfData.specular * DirectBRDFSpecular(brdfData, normalWS, lightDirectionWS, viewDirectionWS));   //----TOON---- This one takes over the smoothness rendering
        ...
        brdf = Toon3(brdf * (1.0 - clearCoatMask * coatFresnel) + brdfCoat * clearCoatMask);   //----TOON---- This one probably takes over the smoothness rendering
#endif // _CLEARCOAT
    }
...
half3 VertexLighting(float3 positionWS, half3 normalWS)
{
    ...
    LIGHT_LOOP_BEGIN(lightsCount)
        ...
        half3 lightColor = light.color * Toon(light.distanceAttenuation);
...
half4 UniversalFragmentBakedLit(InputData inputData, SurfaceData surfaceData)
{
    ...
    return Toon4(CalculateFinalColor(lightingData, surfaceData.albedo, surfaceData.alpha, inputData.fogCoord));
}
